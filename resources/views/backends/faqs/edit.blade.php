...
@section('content')
@php $status = get_statusPost(); var_dump($faqs->slug) @endphp
    ...
    <section class="content">
        <div class="container">
            <div class="head">
                ...
                <h1 class="title">{{__('Edit faqs')}}</h1>
            </div>
            <div class="main">
                @include('notices.index')
                <form action="{{ route('editFaqsAdmin',['id'=>$faqs->id]) }}" name="createFaqs" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                    <div class="row">
                        <div class="col-md-9 content">
                            <div id="frm-title" class="form-group">
                                <label for="title" class="control-label">{{ __('Title') }}<small>(*)</small></label>
                                <input type="text" name="title" value="{{$faqs->title}}" class="form-control" data-error="{{ __('Enter title')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div id="frm-content" class="form-group">
                                <label for="content" class="control-label">{{ __('Content') }}</label>
                                <textarea name="content" rows="10"  class="form-control editor">{!!$faqs->content!!}</textarea>
                            </div>
                            ...
                            <aside id="sb-status" class="card card-outline card-info">
                                ...
                                <div class="card-body">
                                    <select class="form-control select2bs4" name="status">
                                        ...
                                        <option value="{{$key}}"@if($key==$faqs->status){{__(' selected')}}@endif>{{$value}}</option>
                                        @endforeach
                                    </select>
                                    <div class="help-block with-errors"></div>
                                </div>
