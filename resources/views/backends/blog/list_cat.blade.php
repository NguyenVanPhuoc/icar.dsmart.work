...
<div id="list-pages" class="content-wrapper pages">
    ...
    <section class="content">
      ...
      <div class="main">
        <div class="row search-filter">
          <div class="col-md-6 filter">
              <ul class="nav-filter">
                  <li class="active"><a href="{{route('blogCatAdmin')}}">{{__('All')}}</a></li>
                  <li class=""><a href="{{route('storeBlogCatAdmin')}}">{{__('Create')}}</a></li>
              </ul>
          </div>
          <div class="col-md-6 search-form">
              <form name="s" action="{{route('blogCatAdmin')}}" method="GET">
                  <div class="row">
                ...
                <tbody class="tbody">
                  @if(!$blogCates->isEmpty())
                    ...
                    <tr>
                      ...
                      <td class="image"><a href="{{route('editBlogCatAdmin',['id'=>$item->id])}}">{!!image($item->image_id, 100,100, $item->title)!!}</a></td>
                      <td class="title"><a href="{{route('editBlogCatAdmin',['id'=>$item->id])}}">{{$item->title}}</a></td>
                      ...
                      <td class="group-action text-right">
                          <a class="btn btn-info btn-sm" href="{{route('editBlogCatAdmin',['id'=>$item->id])}}"><i class="fas fa-pencil-alt"></i>{{__('edit')}}</a>
                          <a class="btn btn-danger btn-sm" href="{{route('deleteBlogCatAdmin',['id'=>$item->id])}}" data-toggle="modal" data-target="#sideModal" data-direct="modal-top-right"><i class="fas fa-trash"></i>{{__('delete')}}</a>
                      </td>
                    </tr>
                    @endforeach
                  @else
      ...
      @if($s && $status)
            {{ $blogCates->appends(['status'=>$status,'s'=>$s])->links() }}
         @elseif($s)
            {{ $blogCates->appends(['s'=>$s])->links() }}
         @elseif($status)
            {{ $blogCates->appends(['status'=>$status])->links() }}
         @else
            {{ $blogCates->links() }}
         @endif
      </div>
