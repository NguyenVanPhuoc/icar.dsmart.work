...
<div id="list-pages" class="content-wrapper pages">
    ...
    <section class="content">
      ...
      <div class="main">
        <div class="row search-filter">
          <div class="col-md-6 filter">
              <ul class="nav-filter">
                  <li class="active"><a href="{{route('blogAdmin')}}">{{__('All')}}</a></li>
                  <li class=""><a href="{{route('storeBlogAdmin')}}">{{__('Create')}}</a></li>
              </ul>
          </div>
          <div class="col-md-6 search-form">
              <form name="s" action="{{route('blogAdmin')}}" method="GET">
                  <div class="row">
                ...
                <tbody class="tbody">
                  @if(!$blogs->isEmpty())
                    ...
                    <tr>
                      ...
                      <td class="image"><a href="{{route('editBlogAdmin',['id'=>$item->id])}}">{!!image($item->image_id, 100,100, $item->title)!!}</a></td>
                      ...
                      <td class="group-action text-right">
                          <a class="btn btn-info btn-sm" href="{{route('editBlogAdmin',['id'=>$item->id])}}"><i class="fas fa-pencil-alt"></i>{{__('edit')}}</a>
                          <a class="btn btn-danger btn-sm" href="{{route('deleteBlogAdmin',['id'=>$item->id])}}" data-toggle="modal" data-target="#sideModal" data-direct="modal-top-right"><i class="fas fa-trash"></i>{{__('delete')}}</a>
                      </td>
                    </tr>
                    @endforeach
                  @else
      ...
      @if($s && $status)
            {{ $blogs->appends(['status'=>$status,'s'=>$s])->links() }}
         @elseif($s)
