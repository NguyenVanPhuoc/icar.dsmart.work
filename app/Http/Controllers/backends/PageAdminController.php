...
class PageAdminController extends Controller {
	...
	public function edit($id){
		...
		if($page){
			$page = Post::leftJoin('pages','pages.post_id','posts.id')
                ->where('pages.id',$id)
                ->select('posts.id as post_id','pages.id as id','title','slug','excerpt','content','image_id','meta_key','meta_value','type','template','status','user_id')
				->first();
	...
	public function update(Request $request, $id){
        ...
        if ($validator->fails()):
			...
			if($post){
				$meta_fields = $request->meta;
				if(count($meta_fields)>0){
					foreach($meta_fields as $key => $value)
					update_fieldContent($key,$value);
				}
	...
	//change slug
     public function changeSlug(Request $request,$id){
        $message = 'error';
		if($request->ajax()):
            $slug = change_slugPost($request->slug, $id);
